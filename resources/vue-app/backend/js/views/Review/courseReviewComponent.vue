    ...
    <div>
        <data-table :table-heading="tableHeading" :show-add-btn="false">
            <tr v-for="(data, index) in dataList.data" style="font-size: 0.8rem" :key="data.id">
                ...
                <td>
                    <!-- Edit button -->
                    <button v-if="can('course_review_edit')" @click="onClickUpdate(data)" class="btn btn-primary btn-sm" title="Edit" type="button">
                        <i class="fa fa-edit"></i>
                    </button>
                    <!-- Delete button -->
                    <button v-if="can('course_review_delete')" @click="deleteItem(data.id, dataList.current_page, perPage)" class="btn btn-danger btn-sm" title="Delete" type="button">
